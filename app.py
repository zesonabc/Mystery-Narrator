...
def extract_characters_silicon(script, model, key):
    ...
    sys_prompt = "任务：提取中国悬疑剧本角色。规则：1. 默认中国面孔(Asian Chinese face)。2. 输出JSON List: [{'name':'xx','prompt':'...'}]"
    try:
...
def analyze_segments_robust(segments, script_text, char_names, style, res_p, model, key):
    ...
    try:
        ...
        规则: 1. 强制中国背景(Chinese setting)，中国人(Asian Chinese face)。 2. 输出 JSON: {{"segments": [...]}}
        """
...
if st.button("🔍 3. 智能分析"):
    ...
    elif not script_input and not audio: st.error("至少得有个输入")
    else:
        if audio:
