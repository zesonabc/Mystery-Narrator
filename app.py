...
# 1. 页面配置
# ==========================================
st.set_page_config(page_title="MysteryNarrator V23 (新引擎版)", page_icon="🚀", layout="wide")
st.markdown("""
<style>
...
def extract_characters_silicon(script, model, key):
    ...
    try:
        # 这里使用的 model 就是侧边栏选择的新引擎
        res = requests.post(url, json={"model":model,"messages":[{"role":"system","content":"提取【剧情角色】输出JSON列表:[{'name':'xx','prompt':'...'}]"},{"role":"user","content":script}],"response_format":{"type":"json_object"}}, headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"}, timeout=45)
...
def analyze_segments_robust(segments, script_text, char_names, style, res_p, model, key):
    ...
    try:
        ...
        """
        # 这里使用的 model 就是侧边栏选择的新引擎
        res = requests.post("https://api.siliconflow.cn/v1/chat/completions", json={"model":model,"messages":[{"role":"system","content":sys_prompt},{"role":"user","content":input_data}],"response_format":{"type":"json_object"}}, headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"}, timeout=90) # DeepSeek可能慢一点，超时时间加长
...
def generate_image(prompt, size, key):
    try:
        ...
        res = requests.post(
            ...
            json={
                "model": "Qwen/Qwen-Image", # 换成了通义千问生图模型
                "prompt": prompt,
                "image_size": size,
                "batch_size": 1
            }, 
            headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"}, 
            timeout=45 # Qwen生图可能会慢一点点，超时加长
        )
...
with st.sidebar:
    ...
    # 【重点修改】大脑选项更新，DeepSeek V3.2 设为首选
    st.markdown("### 🧠 大脑 (提示词模型)"); 
    model = st.selectbox("选择模型", ["deepseek-ai/DeepSeek-V3.2", "Qwen/Qwen2.5-72B-Instruct"])
    ...
    st.info("🎨 绘图引擎已锁定为: Qwen/Qwen-Image")

st.title("🚀 MysteryNarrator V23 (双引擎升级版)")
st.caption("核心大脑: DeepSeek V3.2 | 绘图引擎: Qwen-Image")
    ...
    if not api_key: st.error("Key?")
        ...
        with st.spinner("处理中..."):
            ...
            if asr and 'segments' in asr and len(asr['segments']) > 0:
                ...
                st.success(f"✅ 听写真实时间轴。")
            else:
                st.session_state.segments = []
                st.warning("⚠️ 使用手动对齐模式 (默认5秒/图)。")
...
if st.session_state.char_df is not None:
    ...
    if st.button("🎬 4. 生成分镜"):
        with st.spinner(f"大脑 ({model}) 正在设计分镜..."):
...
if st.session_state.shot_df is not None and not st.session_state.shot_df.empty:
    ...
    if col1.button("🚀 5. 绘图 (Qwen引擎)"):
        ...
        for i, r in st.session_state.shot_df.iterrows():
            ...
            if i < tot-1: time.sleep(35) # Qwen 建议冷却时间稍长一点
        st.success("完成!")
